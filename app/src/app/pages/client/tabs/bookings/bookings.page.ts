...
export class BookingsPage implements OnInit {
  ...
  get barbershopPhone(): string { return this.resolver.barbershop?.phone ?? ''; }
  get barbershopAddress(): string { return this.resolver.barbershop?.address ?? ''; }

  whatsappUrl(): string {
    const clean = this.barbershopPhone.replace(/\D/g, '');
    return `https://wa.me/${clean}`;
  }

  mapsUrl(): string {
    return `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(this.barbershopAddress)}`;
  }
  ...
  private isInFuture(b: any): boolean {
    const now = new Date();
    const todayStr = now.toISOString().split('T')[0];
    const bookingDateStr = (b.date as string).split('T')[0];

    if (bookingDateStr > todayStr) return true;
    if (bookingDateStr < todayStr) return false;

    // Mismo día: comparar por hora
    const [h, m] = (b.startTime as string).split(':').map(Number);
  ...
  applyFilter(): void {
    ...
    const finalStatuses  = ['CANCELADA', 'COMPLETADA', 'NO_SHOW'];

    if (this.segment === 'upcoming') {
      this.filteredBookings = this.allBookings.filter(b =>
        activeStatuses.includes(b.status) && this.isInFuture(b),
      );
    } else if (this.segment === 'past') {
      this.filteredBookings = this.allBookings.filter(b =>
        finalStatuses.includes(b.status) || !this.isInFuture(b),
      );
    } else {
